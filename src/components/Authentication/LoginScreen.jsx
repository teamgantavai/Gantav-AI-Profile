...
import { ArrowRight, ShieldCheck, Sun, Moon, Mail, Lock, Eye, EyeOff, UserX } from 'lucide-react';
import { signInWithEmail, signInWithGoogle, signInAnon } from '../../firebase';
...
const LoginScreen = ({ onNavigate, isDarkMode: darkProp, setIsDarkMode: setDarkProp }) => {
    ...
    const [loadingType, setLoadingType] = useState(''); // 'email' | 'google' | 'anon'
    ...
    const handleEmailSignIn = async (e) => {
        ...
        try {
            ...
            // onAuthChange in AuthProvider handles the state update → App re-renders
        } catch (err) {
                ...
                <div className="mt-8 text-center">
                    <button onClick={() => onNavigate?.('signup')}
                        ...
                        <span className="flex items-center gap-1 text-sm font-bold text-violet-500 group-hover:text-violet-400 transition-colors">
                            Create one for free <ArrowRight size={15} className="group-hover:translate-x-0.5 transition-transform" />
                        </span>
                    </button>
                </div>
