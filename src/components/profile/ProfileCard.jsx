      ...
      <div className={`relative backdrop-blur-3xl border rounded-[2.5rem] p-8 shadow-2xl transition-all ${isDarkMode ? 'bg-gradient-to-b from-white/5 to-white/[0.01] border-white/10' : 'bg-white border-slate-200 shadow-slate-200/50'}`}>
        <div className="flex flex-col items-center">
          <div className="relative">
            <div className={`w-32 h-32 lg:w-36 lg:h-36 rounded-[2.5rem] border-[3px] p-2 transition-transform duration-700 hover:rotate-3 ${isDarkMode ? 'bg-slate-900/80' : 'bg-slate-100'} border-amber-400`}>
              <img 
          ...
          <div className="mt-7 text-center w-full">
            <div className="flex flex-col items-center gap-2 mb-4">
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setIsEditModalOpen(true)} className={`p-1.5 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-white/10 text-slate-500' : 'hover:bg-slate-100 text-slate-500'}`}>
            ...
            <div className="flex items-center justify-center gap-4 mt-6">
              {socialIcons.map((site) => (
                <a 
                  key={site.id}
                  href={
                    site.id === 'github' ? `https://github.com/${site.url}` :
                    site.id === 'linkedin' ? `https://linkedin.com/in/${site.url}` :
                    site.id === 'x' ? `https://x.com/${site.url}` :
                    site.id === 'discord' ? `https://discord.com/users/${site.url}` :
                    site.id === 'instagram' ? `https://instagram.com/${site.url}` :
                    '#'
                  }
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all border ${isDarkMode ? 'bg-white/[0.03] border-white/10 text-slate-300 hover:bg-white/10' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50 shadow-sm'} ${site.color} hover:scale-110`}
                >
