...
const UserProfile = ({
  ...
  setIsDarkMode,
  firebaseUser,
  forceEditModal = false,
  onSignOut,
}) => {
  const { profile, setProfile } = useUser();

  const [isEditModalOpen, setIsEditModalOpen] = useState(forceEditModal);
  const [isOnline, setIsOnline] = useState(navigator.onLine);
  const [isLoading, setIsLoading] = useState(true);
  const [activeTab, setActiveTab] = useState('profile');
  const [isSaving, setIsSaving] = useState(false);

  // Sync forceEditModal when prop changes
  useEffect(() => {
    if (forceEditModal) setIsEditModalOpen(true);
  }, [forceEditModal]);

  // Profile state — always fresh from Firestore (via useUser)
  const [editForm, setEditForm] = useState(null);

  // Once profile loads from Firestore, populate editForm
  useEffect(() => {
    if (profile) {
      setEditForm({ ...profile });
      setIsLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // If avatar is a base64 data URL (just cropped), upload to Storage
      if (avatarUrl && avatarUrl.startsWith('data:')) {
        const uploaded = await uploadProfilePhoto(firebaseUser.uid, avatarUrl);
      ...
      await updateUserProfile(firebaseUser.uid, dataToSave);
  ...
  const levelValue = Math.max(1, Math.floor(rawPoints / pointsPerLevel) + 1);
  const nextLevelExp = rawPoints > 0
    ? Math.floor((rawPoints % pointsPerLevel) / pointsPerLevel * 100)
    : 0;

  const user = profile
    ? { ...profile, levelValue, nextLevelExp }
    : null;
    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-[#070B16] text-slate-200' : 'bg-[#F1F5F9] text-slate-900'} font-['Sora','sans-serif'] selection:bg-violet-500/30`}>
      ...
      {!isOnline && (
        <div className="fixed bottom-6 left-6 right-6 z-[60] sm:max-w-xs sm:left-auto pointer-events-none animate-in fade-in slide-in-from-bottom-5">
          <div className={`px-4 py-3 rounded-2xl border flex items-center gap-3 shadow-xl backdrop-blur-md ${isDarkMode ? 'bg-[#0e0a1c]/90 border-rose-500/20 text-rose-100' : 'bg-rose-50/95 border-rose-200 text-rose-900'}`}>
          ...
          <div className={`${activeTab === 'profile' ? 'lg:col-span-3' : 'max-w-4xl mx-auto w-full'} flex flex-col gap-6`}>
              ...
              <>
                ...
                {user.activity && user.activity.length > 0 && (
                  <div className={`p-8 rounded-[2.5rem] border backdrop-blur-3xl ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-slate-200'}`}>
                    <h3 className={`text-[11px] font-black uppercase tracking-[0.4em] mb-6 ${isDarkMode ? 'text-slate-500' : 'text-slate-600'}`}>Activity Log</h3>
                    <div className="space-y-3">
                      {user.activity.map((act, idx) => (
                        <div key={act.id || idx} className={`flex items-center justify-between p-4 rounded-2xl border ${isDarkMode ? 'bg-black/20 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                          <div className="flex items-center gap-4">
                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${isDarkMode ? 'bg-violet-500/20' : 'bg-violet-100'}`}>
                              {act.type === 'course' ? '📚' : act.type === 'goal' ? '🎯' : '📝'}
                            </div>
                            <div>
                              <div className={`text-sm font-black ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{act.title}</div>
                              <div className="text-[10px] opacity-60 uppercase font-bold tracking-widest">{act.date} • {act.type}</div>
                            </div>
                          </div>
                          <div className={`text-[10px] font-black uppercase px-3 py-1 rounded-lg ${act.status === 'completed' ? 'bg-emerald-500/20 text-emerald-500' : 'bg-amber-500/20 text-amber-500'}`}>
                            {act.status}
                          </div>
                        </div>
                      ))}
            ...
            {activeTab !== 'profile' && (
              <div className="flex flex-col items-center justify-center py-20 text-center opacity-60">
                ...
                <p className="text-sm font-bold">This section is being built.</p>
                <button onClick={() => setActiveTab('profile')} className="mt-6 text-violet-500 font-black text-xs uppercase tracking-widest">
                  Return to Profile
                </button>
              </div>
            )}
      ...
      {isEditModalOpen && editForm && (
        <EditProfileModal
          ...
          isSaving={isSaving}
          isFirstSetup={forceEditModal && !profile?.profileComplete}
        />
      )}
    </div>
  );
